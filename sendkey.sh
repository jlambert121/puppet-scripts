...
for node in $@; do
   ...
   scp -o 'StrictHostKeyChecking no' "../ssl/private_keys/$node.pem" $username@$node:"/tmp/ssl/private_keys/$node.pem"
   scp -o 'StrictHostKeyChecking no' "../ssl/public_keys/$node.pem" $username@$node:"/tmp/ssl/public_keys/$node.pem"
   scp -o 'StrictHostKeyChecking no' "../ssl/certs/ca.pem" $username@$node:"/tmp/ssl/certs/ca.pem"
